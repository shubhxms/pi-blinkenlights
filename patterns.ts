...
export function parsePattern(source: string): Phase[] {
  ...
  const phases = parts.map((part, index) => {
    ...
    if (durationMs < MIN_PHASE_MS || durationMs > MAX_PHASE_MS) {
      throw new Error(`phase ${index + 1} must last ${MIN_PHASE_MS}-${MAX_PHASE_MS}ms`);
    }
  ...
  if (phases[0]?.on === phases.at(-1)?.on) {
    throw new Error("first and last phases must alternate when the pattern repeats");
  }

  const cycleMs = phases.reduce((total, phase) => total + phase.durationMs, 0);
  if (cycleMs > MAX_CYCLE_MS) throw new Error(`pattern cycle cannot exceed ${MAX_CYCLE_MS}ms`);
  return phases;
}

export function formatPattern(phases: Phase[]): string {
  return phases.map((phase) => `${phase.on ? "on" : "off"} ${phase.durationMs}ms`).join(", ");
}
...
export function resolveSettings(
  ...
  const patterns = Object.fromEntries(
    Object.entries(sources).map(([name, source]) => [name, parsePattern(source)]),
  );
  const activePattern = projectSettings.activePattern ?? globalSettings.activePattern ?? "Classic";
  if (!patterns[activePattern]) throw new Error(`unknown active pattern: ${activePattern}`);
